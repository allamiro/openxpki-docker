...
RUN wget https://packages.openxpki.org/v3/bookworm/openxpki.sources -O - 2>/dev/null | tee /etc/apt/sources.list.d/openxpki.sources && \
    wget https://packages.openxpki.org/v3/bookworm/Release.key -O - 2>/dev/null | gpg -o /usr/share/keyrings/openxpki.pgp --dearmor && \
    apt-get update && \
...
RUN uid=$(/usr/bin/id -u www-data) && \
    /usr/sbin/useradd apache -s /usr/sbin/nologin -b /var/www -g www-data -o -u "$uid" && \
    /usr/sbin/useradd wwwrun -s /usr/sbin/nologin -b /var/www -g www-data -o -u "$uid"
...
RUN mkdir -m755 /run/openxpkid /run/openxpki-clientd && \
    ...
    mkdir -p -m750 /var/log/openxpki-server /var/log/openxpki-client && \
